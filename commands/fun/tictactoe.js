    ...
    async execute(interaction, client) {
        ...
        if (userinfo.bot == true) {
            ...
            let message = new EmbedBuilder()
        		.setTitle('» ERROR')
        		.setDescription('» You cant play TicTacToe with a Bot!')
        		.setFooter({ text: '» ' + version });

            if (interaction.guildLocale == "de") {
                message = new EmbedBuilder()
        		    .setTitle('» FEHLER')
        		    .setDescription('» Du kannst TicTacToe nicht mit einem Bot spielen!')
        		    .setFooter({ text: '» ' + version });
            }
        ...
        if (interaction.user.id == user.toString().replace(/\D/g, '')) {
            ...
            // Send Message
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] [CMD] TICTACTOE : ' + reciever.toString().replace(/\D/g, '') + ' : ' + amount + '€ : SAMEPERSON')
            return await interaction.reply({ embeds: [message.toJSON()], ephemeral: true })
        }
        ...
        return await interaction.reply({ embeds: [message.toJSON()], components: [row] })
    },
};