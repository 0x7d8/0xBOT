...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (stocks < amount) {
            ...
            // Send Message
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] STOCKSELL : ' + stock.toUpperCase() + ' : ' + amount + ' : ' + cash + '€ : NOTENOUGHSTOCKS')
            return interaction.reply({ embeds: [err.toJSON()], ephemeral: true })
        }
