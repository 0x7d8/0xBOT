...
module.exports = {
    ...
    async execute(interaction, client, lang, vote) {
        ...
        if (await bsns.get('g-' + interaction.guild.id + '-1-OWNER') !== interaction.user.id) {
            ...
            // Send Message
            bot.log(false, interaction.user.id, interaction.guild.id, '[CMD] MARKETPRICES : NOTOWNER')
            return interaction.reply({ embeds: [message.toJSON()], ephemeral: true })
        }
        ...
        // Send Message
        bot.log(false, interaction.user.id, interaction.guild.id, '[CMD] MARKETPRICES : ' + itemid.toUpperCase() + ' : ' + newprice + '€')
        return interaction.reply({ embeds: [message.toJSON()], ephemeral: true })
    },
};