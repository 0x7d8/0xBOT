...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (cooldown.get(interaction.user.id.replace(/\D/g, '')) - Date.now() > 0) {
            ...
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] ROB : ONCOOLDOWN : ' + cdown.toFixed(0) + 's');
        ...
        if (userinfo.bot == true) {
            ...
            // Send Message
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] ROB : ' + user + ' : BOT')
            return interaction.reply({ embeds: [err.toJSON()], ephemeral: true })
        }
        ...
        if (money == 35 && moneysnd < 20) {
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] ROB : ' + user + ' : NOTENOUGHMONEY')
        ...
        if (status == false) {
            ...
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] ROB : ' + user + ' : ' + amount + '€ : FAILURE : ' + punishment + '€')
        ...
        // Set Money
        bals.rem(user.replace(/\D/g, ''), amount)
        bals.add(interaction.user.id.replace(/\D/g, ''), amount)
        
        // Send Message
        console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] ROB : ' + user + ' : ' + amount + '€ : SUCCESS')
