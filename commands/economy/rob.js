...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (money == 35 && moneysnd < 20) {
            ...
            return await interaction.reply({ embeds: [notenoughmoney1.toJSON()], ephemeral: true })
     	};
        ...
        if (money == 35 && moneytar < 20) {
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id.replace(/\D/g, '') + ' @ ' + interaction.guild.id + '] [CMD] ROB : ' + user + ' : NOTENOUGHMONEY')
            return await interaction.reply({ embeds: [notenoughmoney2.toJSON()], ephemeral: true })
     	};
        ...
        if (status == false) {
        	...
        	return await interaction.reply({ embeds: [failure.toJSON()] })
        }
        ...
        return await interaction.reply({ embeds: [sucess.toJSON()] })
    },
};