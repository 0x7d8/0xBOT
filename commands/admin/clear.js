...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (amount < 1) {
            ...
            // Send Message
            console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id + ' @ ' + interaction.guild.id + '] CLEAR : NEGATIVEAMOUNT : ' + amount)
        ...
        if (target != null) {
            ...
            await channel.bulkDelete(filtered, true).then(messages => {
                ...
                // Send Message
                console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id + ' @ ' + interaction.guild.id + '] CLEAR : ' + target + ' : ' + amount)
            ...
            await channel.bulkDelete(amount, true).then(messages => {
                ...
                // Send Message
                console.log('[0xBOT] [i] [' + new Date().toLocaleTimeString('en-US', { hour12: false }) + '] [' + interaction.user.id + ' @ ' + interaction.guild.id + '] CLEAR : ' + amount)
                return interaction.reply({ embeds: [message.toJSON()], ephemeral: true })
            })
        }
    },
