...
var yes_default = {
  ...
  async execute(ctx, bet) {
    ...
    let row1 = new import_discord.ActionRowBuilder().addComponents(
      new import_discord.ButtonBuilder().setLabel("\u{1FAA8} ROCK").setCustomId("RPS-1-" + bet).setStyle(import_discord.ButtonStyle.Secondary),
      new import_discord.ButtonBuilder().setLabel("\u{1F4DD} PAPER").setCustomId("RPS-2-" + bet).setStyle(import_discord.ButtonStyle.Secondary),
      new import_discord.ButtonBuilder().setLabel("\u2702\uFE0F SCISSORS").setCustomId("RPS-3-" + bet).setStyle(import_discord.ButtonStyle.Secondary)
    );
    let row2 = new import_discord.ActionRowBuilder().addComponents(
      new import_discord.ButtonBuilder().setEmoji("1024382939020152982").setLabel("CANCEL").setCustomId(`RPS-CANCEL-${bet}`).setStyle(import_discord.ButtonStyle.Danger)
    );
    if (ctx.metadata.language === "de") {
      row1 = new import_discord.ActionRowBuilder().addComponents(
        new import_discord.ButtonBuilder().setLabel("\u2702\uFE0F SCHERE").setCustomId("RPS-3-" + bet).setStyle(import_discord.ButtonStyle.Secondary),
        new import_discord.ButtonBuilder().setLabel("\u{1FAA8} STEIN").setCustomId("RPS-1-" + bet).setStyle(import_discord.ButtonStyle.Secondary),
        new import_discord.ButtonBuilder().setLabel("\u{1F4DD} PAPIER").setCustomId("RPS-2-" + bet).setStyle(import_discord.ButtonStyle.Secondary)
      );
      row2 = new import_discord.ActionRowBuilder().addComponents(
        new import_discord.ButtonBuilder().setEmoji("1024382939020152982").setLabel("ABBRECHEN").setCustomId(`RPS-CANCEL-${bet}`).setStyle(import_discord.ButtonStyle.Danger)
      );
    }
    ...
    let message = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB ROCK PAPER SCISSORS").setDescription(`
				\xBB <@${sender}> is playing Rock Paper Scissors with <@${reciever}>!
				The Bet is **$${bet}**
    ...
    if (ctx.metadata.language === "de") {
      message = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB SCHERE STEIN PAPIER").setDescription(`
					\xBB <@${sender}> spielt mit <@${reciever}> Schere Stein Papier!
					Die Wette ist **${bet}\u20AC**
				`).setFooter({ text: "\xBB " + ctx.client.config.version });
    }
