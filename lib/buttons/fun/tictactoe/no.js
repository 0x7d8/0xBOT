...
var no_default = {
  ...
  async execute(ctx) {
    ...
    let message = new import_discord.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB TICTACTOE").setDescription(`\xBB <@${ctx.interaction.user.id}> said **NO**.`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
    if (ctx.metadata.language === "de") {
      message = new import_discord.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB TICTACTOE").setDescription(`\xBB <@${ctx.interaction.user.id}> hat **NEIN** gesagt.`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
    }
