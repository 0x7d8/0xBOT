...
var cursedimage_default = {
  ...
  async execute(ctx, type) {
    if (!await ctx.bot.settings.get(ctx.interaction.guild.id, "cursedimage")) {
      let message2 = new import_discord.EmbedBuilder().setColor(3604635).setTitle("<:EXCLAMATION:1024407166460891166> \xBB ERROR").setDescription(`\xBB The **\`/cursedimage\`** Command is disabled on this Server!`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      if (ctx.metadata.language === "de") {
        message2 = new import_discord.EmbedBuilder().setColor(3604635).setTitle("<:EXCLAMATION:1024407166460891166> \xBB FEHLER").setDescription(`\xBB Der **\`/cursedimage\`** Befehl ist auf diesem Server deaktiviert!`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      }
    ...
    let message = new import_discord.EmbedBuilder().setColor(3604635).setTitle("<:IMAGE:1024405297579696179> \xBB CURSED IMAGE").setDescription(`
				\xBB Title
    ...
    if (ctx.metadata.language === "de") {
      message = new import_discord.EmbedBuilder().setColor(3604635).setTitle("<:IMAGE:1024405297579696179> \xBB VERST\xD6RENDES BILD").setDescription(`
					\xBB Titel
					\`\`\`${res[0].data.children[0].data.title}\`\`\`
				`).setImage(res[0].data.children[0].data.url).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
    }
