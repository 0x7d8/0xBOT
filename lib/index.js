...
webController.event('notfound', async (ctr) => {
return ctr.printFile('./dashboard/dist/index.html');
});
webController.event('request', async (ctr) => {
...
apiController.event('notfound', async (ctr) => {
return ctr.status(404).print({
success: false,
message: 'ROUTE NOT FOUND'
});
});
apiController.event('request', async (ctr) => {
...
apiController.event('error', async (ctr) => {
...
await apiController.start().then(async (res) => {
...
console.log(await apiController.loadExternalPaths());
});
