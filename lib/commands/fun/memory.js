...
var memory_default = {
  data: new import_discord2.SlashCommandBuilder().setName("memory").setDescription("PLAY MEMORY").setDescriptionLocalizations({
    de: "SPIELE MEMORY"
  ...
  async execute(ctx) {
    ...
    if (user.bot) {
      let message2 = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:EXCLAMATION:1024407166460891166> \xBB ERROR").setDescription(`\xBB You cant play Memory with a Bot!`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      if (ctx.metadata.language === "de") {
        message2 = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:EXCLAMATION:1024407166460891166> \xBB FEHLER").setDescription(`\xBB Du kannst Memory nicht mit einem Bot spielen!`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      }
    ...
    if (ctx.interaction.user.id == user.id) {
      let message2 = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:EXCLAMATION:1024407166460891166> \xBB ERROR").setDescription(`\xBB You cant play Memory with yourself?`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      if (ctx.metadata.language === "de") {
        message2 = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:EXCLAMATION:1024407166460891166> \xBB FEHLER").setDescription(`\xBB Du kannst Memory nicht mit dir selber spielen?`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      }
    ...
    let row = new import_discord.ActionRowBuilder().addComponents(
      new import_discord.ButtonBuilder().setLabel("YES").setCustomId("MEMORY-YES-" + bet).setEmoji("1024382942153285632").setStyle(import_discord.ButtonStyle.Success),
      new import_discord.ButtonBuilder().setLabel("NO").setCustomId("MEMORY-NO-" + bet).setEmoji("1024382939020152982").setStyle(import_discord.ButtonStyle.Danger)
    );
    if (ctx.metadata.language === "de") {
      row = new import_discord.ActionRowBuilder().addComponents(
        new import_discord.ButtonBuilder().setLabel("JA").setCustomId("MEMORY-YES-" + bet).setEmoji("1024382942153285632").setStyle(import_discord.ButtonStyle.Success),
        new import_discord.ButtonBuilder().setLabel("NEIN").setCustomId("MEMORY-NO-" + bet).setEmoji("1024382939020152982").setStyle(import_discord.ButtonStyle.Danger)
      );
    }
    let message = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB MEMORY").setDescription(`
				\xBB <@${ctx.interaction.user.id}> challenges you, <@${user.id}> to a battle of Memory! The Bet is **$${bet}**.
				Do you accept?
    ...
    if (ctx.metadata.language === "de") {
      message = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB MEMORY").setDescription(`
					\xBB <@${ctx.interaction.user.id}> fordert dich, <@${user.id}> zu einem Spiel von Memory heraus! Die Wette ist **${bet}\u20AC**.
					Akzeptierst du?
    ...
    const expiration = async () => {
      ...
      message = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB MEMORY").setDescription(`\xBB The Request expired.`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      if (ctx.metadata.language === "de") {
        message = new import_discord2.EmbedBuilder().setColor(3604635).setTitle("<:GAMEPAD:1024395990679167066> \xBB MEMORY").setDescription(`\xBB Die Anfrage ist abgelaufen.`).setFooter({ text: "\xBB " + ctx.metadata.vote.text + " \xBB " + ctx.client.config.version });
      }
