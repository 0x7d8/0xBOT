...
module.exports = {
  ...
  async code(ctr) {
    ...
    if ((await ctr["@"].bot.votes.get(ctr.body.id + "-A") + 1) % 10 === 0)
      extra = (await ctr["@"].bot.votes.get(ctr.body.id + "-A") + 1) * 1e4 / 2;
...
\xBB Danke f\xFCrs Voten! Du hast **${random}\u20AC** von mir erhalten :)`).setFooter({ text: "\xBB " + ctr["@"].config.version });
    ...
    ;
    let messageBonus = new import_discord.EmbedBuilder().setColor(3604635).setTitle("\xBB VOTING").setDescription(`\xBB Thanks for Voting **${await ctr["@"].bot.votes.get(ctr.body.id + "-A") + 1}** times!
As A Gift I give you extra **$${extra}**!`).setFooter({ text: "\xBB " + ctr["@"].config.version });
    if (await ctr["@"].bot.language.get(ctr.body.id) === "de") {
      messageBonus = new import_discord.EmbedBuilder().setColor(3604635).setTitle("\xBB VOTING").setDescription(`\xBB Danke, dass du **${await ctr["@"].bot.votes.get(ctr.body.id + "-A") + 1}** mal gevotet hast!
Als Geschenk gebe ich dir extra **${extra}\u20AC**!`).setFooter({ text: "\xBB " + ctr["@"].config.version });
    ...
    console.log(`[0xBOT] [i] [${(/* @__PURE__ */ new Date()).toLocaleTimeString("en-US", { hour12: false })}] [WEB] VOTED : ${ctr.body.id} : ${random}\u20AC : DBLCOM`);
