...
module.exports = {
	...
	async code(ctr: webserverInterface) {
		...
		if (!(ctr.reqBody as { user: string }).user) return

		const random = ctr.bot.random(7500, 15000)

		// Calculate Extra
		let extra: number
		if ((await ctr.bot.votes.get((ctr.reqBody as { user: string }).user + '-A')+1) % 10 === 0) extra = ((await ctr.bot.votes.get((ctr.reqBody as { user: string }).user + '-A')+1) * 10000)/2
		...
		if (await ctr.bot.language.get((ctr.reqBody as { user: string }).user) === 'de') {
			...
			.setTitle('» VOTING')
			.setDescription('» Thanks for Voting **' + ((await ctr.bot.votes.get((ctr.reqBody as { user: string }).user + '-A'))+1) + '** times!\nAs A Gift I give you extra **$' + extra + '**!')
			.setFooter({ text: '» ' + ctr.config.version })
		if (await ctr.bot.language.get((ctr.reqBody as { user: string }).user) === 'de') {
			messageBonus = new EmbedBuilder().setColor(0x37009B)
				.setTitle('» VOTING')
				.setDescription('» Danke, dass du **' + ((await ctr.bot.votes.get((ctr.reqBody as { user: string }).user + '-A'))+1) + '** mal gevotet hast!\nAls Geschenk gebe ich dir extra **' + extra + '€**!')
				.setFooter({ text: '» ' + ctr.config.version })
		}
		...
		// Count to Stats
		if ((await ctr.bot.votes.get((ctr.reqBody as { user: string }).user + '-A')+1) % 10 === 0) {
			ctr.bot.money.add(false, (ctr.reqBody as { user: string }).user, extra)
			ctr.client.users.send((ctr.reqBody as { user: string }).user, { embeds: [messageBonus] })
		}; ctr.bot.votes.add((ctr.reqBody as { user: string }).user + '-A', 1)
		ctr.bot.votes.set((ctr.reqBody as { user: string }).user + '-T', Date.now())
