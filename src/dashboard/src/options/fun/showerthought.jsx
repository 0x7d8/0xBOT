...
function Option({ settings, cookies }) {
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          toast({
            ...
            description: <Center>You {calcopt} /showerthought!</Center>,
            status: "success",
            duration: 1500,
            isClosable: true,
            variant: "subtle",
  ...
  useEffect(() => {
    if (settings.showerthought) setText('ENABLED')
  }, [settings])

  return (
    <Menu>
      <Text fontSize="2xl" mt="1rem">SHOWERTHOUGHT COMMAND</Text>
