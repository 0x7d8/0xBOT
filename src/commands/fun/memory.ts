...
export default {
	...
	async execute(ctx: CommandInteraction) {
		...
		let message = new EmbedBuilder().setColor(0x37009B)
  		...
  		.setDescription('» <@' + ctx.interaction.user.id + '> challenges you, <@' + user.id + '> to a battle of Memory! The Bet is **$' + bet + '**.\nDo you accept?\n\n» This Request expires <t:' + (Math.floor(+new Date() / 1000) + 29) + ':R>')
			.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version })

		if (ctx.metadata.language === 'de') {
			message = new EmbedBuilder().setColor(0x37009B)
				.setTitle('<:GAMEPAD:1024395990679167066> » MEMORY')
  			.setDescription('» <@' + ctx.interaction.user.id + '> fordert dich, <@' + user.id + '> zu einem Spiel von Memory heraus! Die Wette ist **' + bet + '€**.\nAkzeptierst du?\n\n» Diese Anfrage wird ungültig <t:' + (Math.floor(+new Date() / 1000) + 29) + ':R>')
				.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version })
		}
		...
		const expiration = async() => {
			...
			if (!ctx.bot.memory.has('TIMEOUT-' + ctx.interaction.user.id + '-' + msg.id)) return
