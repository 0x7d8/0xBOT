...
var amount, reason, message_1, message_2, reasontype, reasonres, row, message;
return __generator(this, function (_a) {
amount = ctx.getOption('amount');
reason = ctx.getOption('reason');
if (amount < 0) {
...
ctx.log(false, "[CMD] BEG : NEGATIVEMONEY : ".concat(amount, "\u20AC"));
...
ctx.log(false, "[CMD] BEG : TOOMUCHMONEY : ".concat(amount, "\u20AC"));
...
.setDescription("\n\t\t\t\t\t\u00BB <@".concat(ctx.interaction.user.id, "> needs Money!\n\t\t\t\t\tTotal Earnings: **$0**\n\t\t\t\t")).setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_2.EmbedBuilder().setColor(0x37009B)
.setTitle('<:DONATE:1024397357988720711> » BETTELN')
.setDescription("\n\t\t\t\t\t\t\u00BB <@".concat(ctx.interaction.user.id, "> braucht Geld!\n\t\t\t\t\t\tInsgesamte Einnahmen: **0\u20AC**\n\t\t\t\t\t")).setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
}
else {
message = new discord_js_2.EmbedBuilder().setColor(0x37009B)
.setTitle('<:DONATE:1024397357988720711> » BEGGING')
.setDescription("\n\t\t\t\t\t\u00BB <@".concat(ctx.interaction.user.id, "> needs Money!\n\t\t\t\t\tTotal Earnings: **$0**\n\t\t\t\t\t*\"").concat(reason.toString(), "\"*\n\t\t\t\t")).setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_2.EmbedBuilder().setColor(0x37009B)
.setTitle('<:DONATE:1024397357988720711> » BETTELN')
.setDescription("\n\t\t\t\t\t\t\u00BB <@".concat(ctx.interaction.user.id, "> braucht Geld!\n\t\t\t\t\t\tInsgesamte Einnahmen: **0\u20AC**\n\t\t\t\t\t\t*\"").concat(reason.toString(), "\"*\n\t\t\t\t\t")).setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
...
ctx.log(false, "[CMD] BEG : ".concat(amount, "\u20AC"));
