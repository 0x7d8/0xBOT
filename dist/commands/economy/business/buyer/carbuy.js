...
var message_1, car, balance, cost, _a, name, missing, message_2, dbcar, message_3, row, message;
...
cost = 490000;
return [3, 8];
case 6: return [4, ctx.bot.businesses.get('g-' + ctx.interaction.guild.id + '-3-PRICE-' + car.toUpperCase())];
case 7:
cost = _b.sent();
_b.label = 8;
case 8:
...
ctx.log(false, "[CMD] CARBUY : ".concat(name.toUpperCase(), " : NOTENOUGHMONEY : ").concat(cost, "\u20AC"));
...
ctx.log(false, "[CMD] CARBUY : ALREADYOWNCAR : ".concat(name));
...
.setDescription("\u00BB Willst du einen **".concat(name, "** f\u00FCr **").concat(cost, "\u20AC** kaufen?"))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
ctx.log(false, "[CMD] CARBUY : ".concat(name.toUpperCase(), " : ").concat(cost, "\u20AC"));
