...
case 1:
money = _a.sent();
ctx.log(false, "[CMD] BALANCE : ".concat(money, "\u20AC"));
return [3, 4];
case 2: return [4, ctx.bot.money.get(user.id)];
case 3:
money = _a.sent();
ctx.log(false, "[CMD] BALANCE : ".concat(user, " : ").concat(money, "\u20AC"));
_a.label = 4;
case 4:
if (!user) {
message = new discord_js_1.EmbedBuilder().setColor(0x37009B)
.setTitle('<:WALLET:1024387370793050273> » YOUR BALANCE')
.setDescription("\u00BB Your Balance is **$".concat(money, "**!"))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_1.EmbedBuilder().setColor(0x37009B)
.setTitle('<:WALLET:1024387370793050273> » DEIN GELDSTAND')
.setDescription("\u00BB Dein Geldstand betr\u00E4gt **".concat(money, "\u20AC**!"))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
}
else {
...
.setDescription("\u00BB The Balance of <@".concat(user.id, "> is **$").concat(money, "**!"))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_1.EmbedBuilder().setColor(0x37009B)
.setTitle('<:WALLET:1024387370793050273> » DER GELDSTAND VON ' + user.username.toUpperCase())
.setDescription("\u00BB Der Geldstand von <@".concat(user.id, "> betr\u00E4gt **").concat(money, "\u20AC**!"))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
}
