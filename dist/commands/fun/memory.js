...
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : BOT"));
...
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : ALREADYLOBBY"));
...
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : NEGATIVEMONEY : ").concat(bet, "\u20AC"));
...
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : ").concat(bet, "\u20AC : SAMEPERSON"));
...
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : NOTENOUGHMONEY"));
...
.setDescription("\n\t\t\t\t\u00BB <@".concat(ctx.interaction.user.id, "> challenges you, <@").concat(user.id, "> to a battle of Memory! The Bet is **$").concat(bet, "**.\n\t\t\t\tDo you accept?\n\t\t\t\t\n\t\t\t\t\u00BB This Request expires <t:").concat(Math.floor(+new Date() / 1000) + 29, ":R>\n\t\t\t")).setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_2.EmbedBuilder().setColor(0x37009B)
.setTitle('<:GAMEPAD:1024395990679167066> » MEMORY')
.setDescription("\n\t\t\t\t\t\u00BB <@".concat(ctx.interaction.user.id, "> fordert dich, <@").concat(user.id, "> zu einem Spiel von Memory heraus! Die Wette ist **").concat(bet, "\u20AC**.\n\t\t\t\t\tAkzeptierst du?\n\t\t\t\t\t\n\t\t\t\t\t\u00BB Diese Anfrage wird ung\u00FCltig <t:").concat(Math.floor(+new Date() / 1000) + 29, ":R>\n\t\t\t\t")).setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : ").concat(bet, "\u20AC"));
...
ctx.log(false, "[CMD] MEMORY : ".concat(user.id, " : EXPIRED"));
