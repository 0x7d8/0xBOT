...
var message, emoji, type, message;
...
.setDescription("\u00BB <@".concat(ctx.interaction.user.id, "> said **NO** to **").concat(amount, "x** ").concat(emoji, " Slots."))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_1.EmbedBuilder().setColor(0x37009B)
.setTitle('<:BOXCHECK:1024401101589590156> » AKTIEN SLOTS KAUFEN')
.setDescription("\u00BB <@".concat(ctx.interaction.user.id, "> hat **NEIN** zu **").concat(amount, "x** ").concat(emoji, " Slots gesagt."))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
ctx.log(false, "[BTN] STOCKUPGRADE : ".concat(amount, "x : ").concat(stock.toUpperCase(), " : DENY"));
