...
.setDescription("\u00BB <@".concat(ctx.interaction.user.id, "> hat **NEIN** zu einer **").concat(name, "** gesagt."))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
}
else {
...
.setDescription("\u00BB <@".concat(ctx.interaction.user.id, "> said **NO** to **").concat(amount, "x** **").concat(name, "**."))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
if (ctx.metadata.language === 'de') {
message = new discord_js_1.EmbedBuilder().setColor(0x37009B)
.setTitle('<:BOXCHECK:1024401101589590156> » GEGENSTÄNDE KAUFEN')
.setDescription("\u00BB <@".concat(ctx.interaction.user.id, "> hat **NEIN** zu **").concat(amount, "x** **").concat(name, "** gesagt."))
.setFooter({ text: '» ' + ctx.metadata.vote.text + ' » ' + ctx.client.config.version });
}
}
ctx.log(false, "[BTN] ITEMBUY : ".concat(amount, "x : ").concat(itemid.toUpperCase(), " : DENY"));
