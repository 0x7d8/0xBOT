...
exports.default = {
    ...
    async execute(ctx, choice) {
        ...
        if (yes + no === 0) {
            ctx.interaction.message.components[0].components[0].data.label = `0 [0%]`;
            ctx.interaction.message.components[0].components[1].data.label = `0 [0%]`;
        }
        else {
            ctx.interaction.message.components[0].components[0].data.label = `${yes} [${Math.round(100 * yes / (yes + no))}%]`;
            ctx.interaction.message.components[0].components[1].data.label = `${no} [${Math.round(100 * no / (yes + no))}%]`;
        }
